...
as `voice` (PocketTTS encodes it against the active model). Uses the shared
pooled httpx client.
"""
...
async def list_voices(base_url: str) -> list[str]:
    try:
        r = await get_client().get(f"{base_url.rstrip('/')}/v1/voices", timeout=5.0)
...
async def synthesize(
    ...
    try:
        r = await get_client().post(url, json=payload, timeout=timeout)
        if r.status_code == 200:
            return r.content
